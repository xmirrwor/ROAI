...
当前开放阶段 0-4，对应急停、蹲起、动作切换、恢复和斜向组合运动。
`max_implemented_stage = 4` 仍是安全门；训练步数越过阶段 4 后会继续停留在
斜向组合运动，不会进入尚未实现的跨障碍。
...
蹲起时为高度深度编码，恢复时为 `[-1, -1]`。动作维数仍为 10。`forced_stage`
默认是 `None`；它只用于
...
未通过项见 `docs/STAGE6_STAGE7_TRAINING_REPORT.md`。
