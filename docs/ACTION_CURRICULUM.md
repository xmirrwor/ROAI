...
当前开放阶段 0-5，对应急停、蹲起、动作切换、恢复、斜向组合运动和跨障碍。
`max_implemented_stage = 5` 是当前安全门；训练步数越过阶段 5 后会继续停留在
跨障碍，不会进入尚未实现的踢球。
...
| 3 | 跌倒恢复雏形 | 倾斜 reset、恢复宽限与超时、连续站稳判定、恢复 reward |
| 4 | 斜向组合运动 | 同时采样 `vx/vy`、航向保持、矢量进度与累计路径误差 reward |
| 5 | 固定低障碍跨越雏形 | 1.2-2.4 cm 横向障碍、距离条件、足端净空/通过/稳定 reward |

64 维观测的最后两维用于技能条件：移动时为步态相位，站立时为 `[1, 0]`，
蹲起时为高度深度编码，恢复时为 `[-1, -1]`。跨障碍保留步态相位，并用障碍阶段
不使用的横向命令观测槽（索引 10）编码归一化前向距离。动作维数仍为 10。`forced_stage`
默认是 `None`；它只用于
...
阶段 4/5 结果见 `docs/STAGE4_STAGE5_TRAINING_REPORT.md`；阶段 6/7、快速运动结果和
未通过项见 `docs/STAGE6_STAGE7_TRAINING_REPORT.md`；跨障碍和完整动作串联见
`docs/STAGE8_AND_FULL_SEQUENCE_REPORT.md`。
